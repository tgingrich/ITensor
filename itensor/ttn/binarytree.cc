...
call .position(j) or .orthogonalize() to set ortho center");
 ...
 //    return eltC(inH[0]);
  ...
  void
  updateOp(ITensor & tens, Matrix const& mat, int r, int c) {
    if (c) {
      tens.set(1, 1, r, c, mat(0, 0));
      tens.set(1, 2, r, c, mat(0, 1));
      tens.set(2, 1, r, c, mat(1, 0));
      tens.set(2, 2, r, c, mat(1, 1));
    } else {
      tens.set(1, 1, r, mat(0, 0));
      tens.set(1, 2, r, mat(0, 1));
      tens.set(2, 1, r, mat(1, 0));
      tens.set(2, 2, r, mat(1, 1));
    }
  }
  ...
  MPO_ASEP(SiteSet const& sites, std::vector<Real> plist, std::vector<Real> qlist, Real lambda)
  {
    ...
    if ((int)plist.size() == length(W) && (int)qlist.size() == length(W)) {
      ...
      for(int b = 1; b <= length(W); ++b) {
        updateOp(W.ref(b), Id, 1, 1);
        updateOp(W.ref(b), ap, 2, 1);
        updateOp(W.ref(b), v, 3, 1);
        updateOp(W.ref(b), n, 4, 1);
        updateOp(W.ref(b), am, 5, 1);
        updateOp(W.ref(b), plist[b % length(W)] * std::exp(lambda) * am, 1, 2);
        updateOp(W.ref(b), -plist[b % length(W)] * n, 1, 3);
        updateOp(W.ref(b), -qlist[b % length(W)] * v, 1, 4);
        updateOp(W.ref(b), qlist[b % length(W)] * std::exp(-lambda) * ap, 1, 5);
        updateOp(W.ref(b), (b == 1 ? -1 : 1) * Id, 6, 6);
      }
      ...
      updateOp(W.ref(1), plist[0] * (std::exp(lambda) * ap - v) + qlist[0] * (std::exp(-lambda) * am - n), 1);
      updateOp(W.ref(1), plist[1] * std::exp(lambda) * am, 2);
      updateOp(W.ref(1), -plist[1] * n, 3);
      updateOp(W.ref(1), -qlist[1] * v, 4);
      updateOp(W.ref(1), qlist[1] * std::exp(-lambda) * ap, 5);
      updateOp(W.ref(1), Id, 6);
      for(int b = 2; b < length(W); ++b) {
        updateOp(W.ref(b), Id, 1, 1);
        updateOp(W.ref(b), ap, 2, 1);
        updateOp(W.ref(b), v, 3, 1);
        updateOp(W.ref(b), n, 4, 1);
        updateOp(W.ref(b), am, 5, 1);
        updateOp(W.ref(b), plist[b] * std::exp(lambda) * am, 6, 2);
        updateOp(W.ref(b), -plist[b] * n, 6, 3);
        updateOp(W.ref(b), -qlist[b] * v, 6, 4);
        updateOp(W.ref(b), qlist[b] * std::exp(-lambda) * ap, 6, 5);
        updateOp(W.ref(b), Id, 6, 6);
      }
      updateOp(W.ref(length(W)), Id, 1);
      updateOp(W.ref(length(W)), ap, 2);
      updateOp(W.ref(length(W)), v, 3);
      updateOp(W.ref(length(W)), n, 4);
      updateOp(W.ref(length(W)), am, 5);
      updateOp(W.ref(length(W)), qlist[length(W)] * (std::exp(-lambda) * ap - v) + plist[length(W)] * (std::exp(lambda) * am - n), 6);
    } else {
